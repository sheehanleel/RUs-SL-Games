...
class TestLinkedList(unittest.TestCase):
    ...
    def test_insert_at_beginning_forwards(self):
        ...
        expected = "Player: Mikkel - uid: 0 <-> Player: Chrissandra - uid: 1 <-> Player: Lee - uid: 2 <-> Player: Brian - uid: 5 <-> None"
        self.assertEqual(output, expected)

    def test_insert_at_beginning_backwards(self):
        self.head = self.ll.insert_at_beginning(self.new_player)
        output = self.ll.traverse_backwards()
        expected = "Player: Brian - uid: 5 <-> Player: Lee - uid: 2 <-> Player: Chrissandra - uid: 1 <-> Player: Mikkel - uid: 0 <-> None"
        self.assertEqual(output, expected)
    ...
    def test_insert_after_node_forwards(self):
        ...
        output = self.ll.traverse()
        expected = "Player: Chrissandra - uid: 1 <-> Player: Lee - uid: 2 <-> Player: Drey - uid: 3 <-> Player: Brian - uid: 5 <-> None"
        self.assertEqual(output, expected)
    ...
    def test_insert_after_node_backwards(self):
        ...
        self.ll.insert_after_node(node, self.new_player_1)
        output = self.ll.traverse_backwards()
        expected = "Player: Brian - uid: 5 <-> Player: Drey - uid: 3 <-> Player: Lee - uid: 2 <-> Player: Chrissandra - uid: 1 <-> None"
        self.assertEqual(output, expected)
    ...
    def test_insert_before_node_forwards(self):
        ...
        output = self.ll.traverse()
        expected = "Player: Chrissandra - uid: 1 <-> Player: Lee - uid: 2 <-> Player: Temu - uid: 4 <-> Player: Brian - uid: 5 <-> None"
        self.assertEqual(output, expected)
    ...
    def test_insert_before_node_backwards(self):
        ...
        self.ll.insert_before_node(node, self.new_player_2)
        output = self.ll.traverse_backwards()
        expected = "Player: Brian - uid: 5 <-> Player: Temu - uid: 4 <-> Player: Lee - uid: 2 <-> Player: Chrissandra - uid: 1 <-> None"
        self.assertEqual(output, expected)
    
    def test_insert_at_end_forwards(self):
        self.ll.insert_at_end(self.new_player_3)
        output = self.ll.traverse()
        expected = "Player: Chrissandra - uid: 1 <-> Player: Lee - uid: 2 <-> Player: Brian - uid: 5 <-> Player: Goofy - uid: 6 <-> None"
        self.assertEqual(output, expected)

    def test_insert_at_end_backwards(self):
        self.ll.insert_at_end(self.new_player_3)
        output = self.ll.traverse_backwards()
        expected = "Player: Goofy - uid: 6 <-> Player: Brian - uid: 5 <-> Player: Lee - uid: 2 <-> Player: Chrissandra - uid: 1 <-> None"
        self.assertEqual(output, expected)

    def test_delete_at_beginning_forwards(self):
        self.head = self.ll.delete_at_beginning()
        output = self.ll.traverse()
        expected = "Player: Lee - uid: 2 <-> Player: Brian - uid: 5 <-> None"
        self.assertEqual(output, expected)
